...
#Auto correlation function (ACF) plots display correlation between a series & its lags
#Series correlated with lags indicates seasonal trends/components
acf(parlight, lag.max=45)
pacf(parlight, lag.max=45)
...
#Test out the auto-generated ARIMA model values:
#Residuals should be normally distributed
tsdisplay(residuals(arima.nee5), lag.max=45)
...
par(mfrow=c(1,1))
plot(nee , typ="l", col="blue"); lines(fitted(arima.nee5),col="red")

#Forecast NEE for 30 days:
plot(forecast(arima.nee5, h=30))
